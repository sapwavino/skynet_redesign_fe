  ...
  <div class="hidden md:block">
    ...
    <div>
      ...
      <div class="block w-1/5 mx-auto mt-5 col-span-3">
        <select id="currency"
                class="h-12 border-2 border-gray-400 dark:text-gray-300 text-base rounded-lg block w-full py-2.5 px-4 focus:outline-none font-bold"
      ...
      <div v-if="tab === 'gpu'" class="grid grid-cols-3 gap-5 p-10">
        <div v-if="gpuPlans.length === 0" class="col-span-3 h-[80vh] flex flex-col justify-center">
          ...
          <p class="text-center mb-5">You can opt-in for an early bird notification once there are servers
            available.</p>
          <input type="email" v-model="earlyBirdEmail"
                 class="outline-customGold border border-customGold w-1/4 mx-auto rounded-2xl mb-3.5 p-2"
                 placeholder="your_email@yourwebsite.com">
