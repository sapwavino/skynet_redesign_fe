  ...
  methods: {
    ...
    removeItem(id) {
      ...
      cartService.removeItemFromCart(id)
          .then(() => { // Use .then for the promise
            this.$store.commit('REMOVE_CART_ITEM', id); // Commit a mutation to remove the item locally
            // Optionally, you can still refresh the entire cart in the background
          ...
          .catch(() => {
            // Handle error, maybe revert the local change
            // this.loadingCart = false;
          });
    }
  },
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 w-full">
                  <div class="md:col-span-2">
                    <div class="flex flex-col">
                      ...
                      <p class="border-btn-error-sm w-2/3  md:w-1/2 mt-1 font-bold tracking-wider text-sm cursor-pointer text-red-700"
                  ...
                  <div v-if="item.price"
                       class="flex items-center md:justify-center md:justify-end max-md:mt-3 h-full"
                  >
                    <p
                        class="font-bold text-xl leading-8 text-center transition-all duration-300 group-hover:text-customGold"
                    >
                      {{ $store.getters.preferredCurrencySymbol }}
                      {{ formatNumber(convertPrice($store.state.preferredCurrency, item.price)) }}</p>
          ...
          >
            ...
            <div class="mt-8">
              <form>
                ...
                <button class="btn-base w-full mx-auto my-5"
                        @click="$router.back()"
                >
                  Continue Shopping
                </button>
